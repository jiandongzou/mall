                  ...
                  <li v-for="item in  goodlist">
                    <div class="cart-tab-1">
                      <div class="cart-item-pic">
                        <img v-bind:src="'../../static/img/'+ item.productImage" alt="item.productName">
                      </div>
              ...
              <div class="price-count">
                <ul>
                  <li>
                    ...
                    <span>{{subTotal|currencyunit("￥")}}</span>
                  </li>
                  <li>
                    <span>Shipping:</span>
                    <span>{{shipping|currencyunit("￥")}}</span>
                  </li>
                  <li>
                    <span>Discount:</span>
                    <span>{{discount|currencyunit("￥")}}</span>
                  </li>
                  <li>
                    <span>Tax:</span>
                    <span>{{tax|currencyunit("￥")}}</span>
                  </li>
                  <li class="order-total-price">
                    <span>Order total:</span>
                    <span>{{orderCount|currencyunit("￥")}}</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          getorderList(){
             axios.post("/goodlist/orderlist").then((res)=>{
               ...
               if( resData.status==0){
                 ...
                 this.orderCount=this.subTotal+this.shipping+this.tax-this.discount
               }
             })
          },
          payment(){
             ...
             axios.post("/goodlist/payment",{
               addressId:addressId,
               orderCount:orderCount
             }).then((res)=>{
               var resData=res.data;
               if(resData.status==0){
                  this.$router.push("/goods/ordersucess?orderid="+resData.result.orderId)
               }
             })
          }
        }
